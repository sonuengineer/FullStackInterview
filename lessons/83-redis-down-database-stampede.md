...
## 6. Real Example (ThePipingMart)

ThePipingMart's supplier search already does the first step right: **a Redis error is treated as a cache miss**, the search falls back to MongoDB, and a failed cache write never fails the request. What it doesn't have yet is a limit on how many fallback queries hit MongoDB at once - that's the layer I'd add next.
