// Embeds every ai/NN-*.md page into index.html as a network-data block,
// so the dashboard's "AI" tab works even when opened as a local file.
// Re-run after adding or editing a page:   node tools/build-ai.mjs
...
const START = '<!--AI-DATA-START-->';
const END = '<!--AI-DATA-END-->';
...
const html = blocks.map((b) =>
  '<script type="text/markdown" class="ai-data" data-n="' + b.n + '" data-title="' + b.title + '">\n' +
  b.md + (b.md.endsWith('\n') ? '' : '\n') + '</script>').join('\n');
...
console.log('AI pages embedded in index.html:');
